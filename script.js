...
$(document).ready(function() {
	...
	var socketIO = io('over-skin.ru:8303');
		...
		// currency
  		...
  		socketIO.on('informers', function(data){
			...
			if(data.inf4 && $('.jackpotNum').length) {
				...
				$('.jackpotNum').text(jackpotThousands + ' т.р.');
			}
...
sec--;
  		...
  		socketIO.on('0', function(data){
			...
			lastticket = ntk;
			var tick = ntk-ddk;
			text.innerHTML = data.user + " вложил <b>" + data.itemname + "</b> (~" + data.cost + " " + currency + ")<br />Билеты от <b>#"+ddk+"</b> до <b>#"+ntk+"</b> <div class='bilet'><span>"+tick+"</span></div>";
			...
			$('title').text(data.jackpot.toFixed(0) + currency + ' - Over-Skin.Ru');
...
$('.winner-cost-valuta').text(' ' + currency);			// Tape 
	...
	var buildHistoryPage = function(history, historyOrder, commission) {
		...
		$.each(historyOrder, function(i, index) {
			...
			$.each(el.items, function(index1, item) {
				itemsHistory += '<li>\
								<a class="history-item-prize" data-image="http://steamcommunity-a.akamaihd.net/economy/image/'+ item.image + '/85fx70f" data-title="' + item.itemname + '" data-price="~ ' + item.cost + '" data-color="#' +  item.background_color + '">\
			    ...
			    <div class="history-item-top">\
			        ...
			        <div class="history-item-top-left">\
			            ...
			            <div class="history-item-desc">\
			                <div class="history-item-title">' + el.winnername + '</div>\
