...
sec--;
  		...
  		socketIO.on('0', function(data){
			...
			lastticket = ntk;
			text.innerHTML = data.user + " вложил <b>" + data.itemname + "</b> (~" + data.cost + " " + currency + ")<br />Билеты от <b>#"+ddk+"</b> до <b>#"+ntk+"</b>";
...
$('.winner-cost-valuta').text(' ' + currency);
		...
		socketIO.on('trade-link', function(data){
  		...
  		socketIO.on('playersUnique', function(data){
			...
			// for admins
			if (getSteamID() == data.superadmin && data.superadmin.length > 0) {
